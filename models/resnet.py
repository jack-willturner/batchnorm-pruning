...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.bn1   = bn.BatchNorm2dEx(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2   = bn.BatchNorm2dEx(planes)
        ...
        # TODO change shortcuts to extended batchnorm
        #if stride != 1 or in_planes != self.expansion*planes:
        #    self.shortcut = nn.Sequential()
...
class BasicBlockCompressed(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv2 = nn.Conv2d(planes[1], planes[2], kernel_size=3, stride=1, padding=1, bias=False)
...
class ResNetCompressed(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride))
            self.in_planes = planes[3] * block.expansion
        return nn.Sequential(*layers)
